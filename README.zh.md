...
**Q: 处理长视频的效果如何，会不会很慢？**
A: 对于长视频，我们采用 MapReduce 策略：先将视频内容进行拆分，然后并发地调用 LLM 进行分析，最后通过主 LLM 对信息进行对齐、聚合并产出最终结果，以最大程度提高分析效率。处理一个小时的视频，若已配置云端 ASR，整体耗时大约在 1 到 2 分钟；若使用本地 faster-whisper 转写，耗时大约在 15 到 20 分钟（取决于本机硬件与性能）。
